...
public class RP_OF_OP_FUNCDao extends GenericDaoJpaImpl<RP_OF_OP_FUNC, Integer>
	...
	public List<RP_OF_OP_FUNC> getbyid(Integer id_of_cab, String user, ArrayList<String> estado) {
		...
		Query query = entityManager.createQuery("Select a,b,c, "
				+ "(select distinct v.DATA_INI_M2 from RP_OF_PREP_LIN v where a.ID_OP_CAB = v.ID_OP_CAB), "
				+ "(select distinct v.HORA_INI_M2 from RP_OF_PREP_LIN v where a.ID_OP_CAB = v.ID_OP_CAB), "
				+ "(select distinct v.DATA_FIM_M2 from RP_OF_PREP_LIN v where a.ID_OP_CAB = v.ID_OP_CAB), "
				+ "(select distinct v.HORA_FIM_M2 from RP_OF_PREP_LIN v where a.ID_OP_CAB = v.ID_OP_CAB) "
	...
	public List<RP_OF_OP_FUNC> checkuser(String user) {
		...
		Query query = entityManager.createQuery("Select a,b,c from RP_OF_OP_FUNC a,RP_OF_OP_CAB b, RP_OF_CAB c "
	...
	public List<RP_OF_OP_FUNC> getallUsersTEMPPREP(Integer id) {

		Query query = entityManager
				.createQuery("select a, (select distinct d.HORA_INI_M2 from RP_OF_PREP_LIN d where d.ID_OP_CAB = b.ID_OP_CAB), "
						+ "(select distinct d.DATA_INI_M2 from RP_OF_PREP_LIN d where d.ID_OP_CAB = b.ID_OP_CAB), "
						+ "(select distinct d.DATA_FIM_M2 from RP_OF_PREP_LIN d where d.ID_OP_CAB = b.ID_OP_CAB), "
						+ "(select distinct d.HORA_FIM_M2 from RP_OF_PREP_LIN d where d.ID_OP_CAB = b.ID_OP_CAB) "
						+ " from RP_OF_OP_FUNC a, RP_OF_OP_CAB b, RP_OF_CAB c where c.ID_OF_CAB = :id and b.ID_OF_CAB = c.ID_OF_CAB and b.ID_OP_CAB = a.ID_OP_CAB");
		query.setParameter("id", id);
		List<RP_OF_OP_FUNC> utz = query.getResultList();
		return utz;
