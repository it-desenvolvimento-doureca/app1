...
public class RP_OF_DEF_LINDao extends GenericDaoJpaImpl<RP_OF_DEF_LIN, Integer>
	...
	public static List<RP_OF_DEF_LIN> sum_defeitos(RP_OF_DEF_LIN data) {
		...
		Query query = RP_OF_DEF_LINDao.entityManager.createQuery("select a.ID_DEF_LIN, "
				+ "(select sum(QUANT_DEF) from RP_OF_DEF_LIN a where a.ID_OP_LIN = " + data.getID_OP_LIN() + " and a.ID_REF_ETIQUETA =" + data.getID_REF_ETIQUETA() + ") as total_ref, "
				+ "(select sum(QUANT_DEF) from RP_OF_DEF_LIN a where a.ID_OP_LIN = " + data.getID_OP_LIN() + " ) as total_eti "
						+ "from RP_OF_DEF_LIN a where a.ID_DEF_LIN = :id");
