...
/**
 * Serviço que sincroniza as referências de uma OF com o ERP: adiciona novas
 * refs PF/COMP e remove as que já não existem. Chamado por SIIP.java em POST
 * /siip/atualizarReferencias/{id_of_cab}
 */
@Stateless
public class AtualizarReferenciasService {
	...
	public Map<String, List<String>> executar(Integer idOfCab, String ipPosto) throws SQLException {
		...
		List<Object[]> cabData = em
				.createNativeQuery("SELECT OF_NUM, ESTADO, SEC_NUM, SEC_DES, ID_UTZ_CRIA, NOME_UTZ_CRIA, OP_COD "
						+ "FROM RP_OF_CAB WHERE ID_OF_CAB = " + idOfCab)
				.getResultList();

		if (cabData.isEmpty())
			throw new RuntimeException("RP_OF_CAB nao encontrado: " + idOfCab);
		...
		if (ofData.isEmpty())
			throw new RuntimeException("OF nao encontrada no ERP: " + ofNum);
		...
		List<Object> opCabResult = em
				.createNativeQuery("SELECT b.ID_OP_CAB FROM RP_OF_CAB a "
						+ "INNER JOIN RP_OF_OP_CAB b ON a.ID_OF_CAB = b.ID_OF_CAB " + "WHERE a.ID_OF_CAB = " + idOfCab)
				.getResultList();
		if (opCabResult.isEmpty())
			throw new RuntimeException("RP_OF_OP_CAB nao encontrado: " + idOfCab);
		Integer idOpCab = ((Number) opCabResult.get(0)).intValue();

		// Refs PF actuais → ref_num → id_op_lin
		List<Object[]> curPF = em
				.createNativeQuery("SELECT ID_OP_LIN, REF_NUM, QUANT_OF FROM RP_OF_OP_LIN WHERE ID_OP_CAB = " + idOpCab)
				.getResultList();
		Map<String, Integer> currentPFMap = new LinkedHashMap<>();
		Integer quantOf = 0;
		for (Object[] row : curPF) {
			currentPFMap.put(str(row[1]), num(row[0]));
			if (quantOf == 0 && row[2] != null)
				quantOf = num(row[2]);
		}

		// Refs COMP actuais (OFs filhas) → ref_num → id_of_cab filho
		List<Object[]> curCOMP = em.createNativeQuery("SELECT c.ID_OF_CAB, e.REF_NUM " + "FROM RP_OF_CAB c "
				+ "INNER JOIN RP_OF_OP_CAB d ON c.ID_OF_CAB = d.ID_OF_CAB "
				+ "INNER JOIN RP_OF_OP_LIN e ON d.ID_OP_CAB = e.ID_OP_CAB " + "WHERE c.ID_OF_CAB_ORIGEM = " + idOfCab)
				.getResultList();
		Map<String, Integer> currentCOMPMap = new LinkedHashMap<>();
		for (Object[] row : curCOMP)
			currentCOMPMap.put(str(row[1]), num(row[0]));

		// 7. Diferenças PF
		Set<String> newPFSet = new HashSet<>();
		for (HashMap<String, String> pf : erpPFRefs)
			newPFSet.add(pf.get("PROREF"));

		List<HashMap<String, String>> toAddPF = new ArrayList<>();
		for (HashMap<String, String> pf : erpPFRefs) {
			if (!currentPFMap.containsKey(pf.get("PROREF")))
				toAddPF.add(pf);
		}
		List<String> toRemovePF = new ArrayList<>();
		for (String ref : currentPFMap.keySet()) {
			if (!newPFSet.contains(ref))
				toRemovePF.add(ref);
		}
		...
		for (String ref : currentCOMPMap.keySet()) {
			if (!validCOMPs.contains(ref))
				toRemoveCOMP.add(ref);
		}
		...
		if (!opCod.isEmpty()) {
			...
			List<Object> confOp = em
					.createNativeQuery("SELECT ID_OP_SEC FROM RP_CONF_OP WHERE ID_OP_PRINC = '" + esc(opCod) + "'")
					.getResultList();
			for (Object row : confOp)
				famOp.add(str(row).trim());
		}
		...
		for (HashMap<String, String> ref : toAddPF) {
			...
			Query qLin = em.createNativeQuery("INSERT INTO RP_OF_OP_LIN "
					...
					+ "GESCOD,PROQTEFMT,TIPO_PECA,NCLQTE,VERSAO_MODIF,CONFIRMAR_ZERO) VALUES (" + idOpCab + ",'"
					+ esc(ref.get("PROREF")) + "','" + esc(trim(ref.get("PRODES1")) + " " + trim(ref.get("PRODES2")))
					+ "'," + sqlStr(ref.get("INDREF")) + "," + sqlStr(ref.get("VA1REF")) + ","
					+ sqlStr(ref.get("VA2REF")) + "," + sqlNum(ref.get("INDNUMENR")) + "," + (quantOf * nclqte)
					+ ",0,0," + perc + "," + "0,0,0,0," + sqlStr(ref.get("GESCOD")) + ","
					+ boolInt(ref.get("PROQTEFMT")) + ",'" + esc(ref.get("PROTYPCOD")) + "'," + nclqte
					+ ",0,null); SELECT @@IDENTITY");
			Integer idNewOpLin = ((BigDecimal) qLin.getSingleResult()).intValue();

			// RP_OF_OUTRODEF_LIN × 4 (id_DEF_OUTRO 1-4)
			for (int i = 1; i <= 4; i++) {
				em.createNativeQuery("INSERT INTO RP_OF_OUTRODEF_LIN (ID_OP_LIN,ID_UTZ_CRIA,ID_DEF_OUTRO) VALUES ("
						+ idNewOpLin + ",'" + esc(idUtzCria) + "'," + i + ")").executeUpdate();
			}
					...
					if (quacod != null && quacod.length() >= 4 && !"00".equals(quacod.substring(2, 4))) {
						em.createNativeQuery(
								"INSERT INTO RP_OF_LST_DEF (ID_OP_LIN,COD_DEF,DESC_DEF,DATA_HORA_MODIF,ID_UTZ_CRIA) VALUES ("
										+ idNewOpLin + ",'" + esc(quacod) + "','" + esc(def.get("QUALIB"))
										+ "',GETDATE(),'" + esc(idUtzCria) + "')")
								.executeUpdate();
					}
				}
			}
		...
		for (HashMap<String, String> filho : toAddCOMP) {
			...
			// RP_OF_CAB filho
			Query qCab = em.createNativeQuery("INSERT INTO RP_OF_CAB "
			...
			// RP_OF_OP_LIN
			em.createNativeQuery("INSERT INTO RP_OF_OP_LIN "
					...
					+ "GESCOD,PROQTEFMT,TIPO_PECA,NCLQTE) VALUES (" + idNewOpCab + ",'" + esc(filho.get("PROREF"))
					+ "','" + esc(trim(filho.get("PRODES1")) + " " + trim(filho.get("PRODES2"))) + "',"
					+ (quantOf * nclqte) + ",0,0," + sqlDoubleOrNull(filho.get("ZPAVAL")) + "," + "0,0,0,0,"
					+ sqlStr(filho.get("GESCOD")) + "," + boolInt(filho.get("PROQTEFMT")) + ",'"
					+ esc(filho.get("PROTYPCOD")) + "'," + nclqte + ")").executeUpdate();
	...
	private String sqlStr(String v) {
		return (v != null && !v.isEmpty()) ? "'" + esc(v) + "'" : "NULL";
	}

	private String sqlNum(String v) {
		return (v != null && !v.isEmpty()) ? v : "NULL";
	}

	private int boolInt(String v) {
		return "1".equals(v) ? 1 : 0;
	}

	/**
	 * Devolve o valor numérico ou NULL se ZPAVAL é null/vazio (PERC_OBJETIV deve
	 * ser NULL se não definido)
	 */
